...
import { Plus, Pencil, Trash2, Search, Users, Loader2, AlertCircle, Mail, Phone } from 'lucide-react';
import { formatDate } from '../lib/utils';

interface CustomerForm { name: string; email: string; phone: string; address: string; }
...
export function CustomersPage() {
  ...
  async function handleSave() {
    ...
    if (!error) { setDialogOpen(false); fetchCustomers(); }
  ...
  async function handleDelete(id: string) {
    ...
    if (!error) { setDeleteId(null); fetchCustomers(); }
    else setError(error.message);
  }
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <Card className="glass border-border/50">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-sm text-muted-foreground ml-auto">{filtered.length} customers</CardTitle>
          </div>
        </CardHeader>
              ...
              <TableBody>
                {filtered.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-muted-foreground text-sm max-w-[200px] truncate">{c.address ?? '—'}</TableCell>
                    <TableCell className="text-muted-foreground text-xs">{formatDate(c.created_at)}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => openEdit(c)}><Pencil className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => setDeleteId(c.id)} className="text-destructive hover:text-red-400">
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">This will permanently delete this customer and may affect related sales records.</p>
          <DialogFooter>
